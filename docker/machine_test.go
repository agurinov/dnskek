...
func TestNewMachine(t *testing.T) {
	...
	if machine.Name != dm.Name {
		t.Errorf("Expected `%s`, got `%s`", dm.Name, machine.Name)
	}
	if machine.DriverName != dm.DriverName {
		t.Errorf("Expected `%s`, got `%s`", dm.DriverName, machine.DriverName)
	}
	if machine.State != dm.State {
		t.Errorf("Expected `%s`, got `%s`", dm.State, machine.State)
	}
	if machine.URL != dm.URL {
		t.Errorf("Expected `%s`, got `%s`", dm.URL, machine.URL)
	}
	if machine.DockerVersion != dm.DockerVersion {
		t.Errorf("Expected `%s`, got `%s`", dm.DockerVersion, machine.DockerVersion)
	}
	if !machine.IP.Equal(dm.IP) {
		t.Errorf("Expected `%s`, got `%s`", dm.IP, machine.IP)
	}
}
